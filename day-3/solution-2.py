...
def is_number_adjecent_to_symbol(x, y, number):
    ...
    # same line as number:
    if x>0:
        if is_symbol_star(file_lines_list[y][x-1]):
            key = f"{y},{x-1}"
            if key in gears_dict.keys():
                gears_dict[key].append(number)
            else:
                gears_dict.update({key: [number]})

    fol_char_x_idx = x+number_len
    if fol_char_x_idx<width:
        if is_symbol_star(file_lines_list[y][x+number_len]):
            key = f"{y},{x+number_len}"
            if key in gears_dict.keys():
                gears_dict[key].append(number)
            else:
                gears_dict.update({key: [number]})
    ...
    if y>0:
        ...
        for index in range(from_idx, until_idx+1):
            if is_symbol_star(file_lines_list[y-1][index]):
                key = f"{y-1},{index}"
                if key in gears_dict.keys():
                    gears_dict[key].append(number)
                else:
...
result_list = []
for values in gears_dict.values():
    if len(values) == 2:
        result_list.append(values[0]*values[1])
